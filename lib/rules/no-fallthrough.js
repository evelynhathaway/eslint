 ...
 * @param {ASTNode} node - A SwitchCase node or token to get comments.
    ...
    create(context) {
        ...
        return {
            ...
            SwitchCase(node) {
                ...
                /*
                 * Checks whether or not there is a fallthrough comment before node or before end of the block.
                 * And reports the previous fallthrough node if that does not exist.
                 */
                if (fallthroughCase) {
                    const beforeBlockEndToken = sourceCode.getLastToken(fallthroughCase);
                    const validNodeOrBlockCommentExists = (
                        hasFallthroughComment(node, context, fallthroughCommentPattern) ||
                        hasFallthroughComment(beforeBlockEndToken, context, fallthroughCommentPattern)
                    );

                    if (!validNodeOrBlockCommentExists) {
                        context.report({
